...
# Number of individuals in group i and j.
Ti <- apply(obs,1,sum)
Tj <- apply(obs,2,sum)
...
# Assume teaching is always successful.
...
#     dx1 <- -x[1]*(alpha[1,1]*tau[1,1]*x[3] + alpha[1,2]*tau[1,2]*x[4]) + nu[1]*x[3]
#     dx2 <- -x[2]*(alpha[2,2]*tau[2,2]*x[4] + alpha[2,1]*tau[2,1]*x[3]) + nu[2]*x[4]
#     dx3 <-  x[1]*(alpha[1,1]*tau[1,1]*x[3] + alpha[1,2]*tau[1,2]*x[4]) - nu[1]*x[3]
#     dx4 <-  x[2]*(alpha[2,2]*tau[2,2]*x[4] + alpha[2,1]*tau[2,1]*x[3]) - nu[2]*x[4]
...
# # model.out1 <- as.data.frame(lsoda(x0,times,sis2))
