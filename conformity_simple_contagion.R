...
h = matrix( c(0.8, 0.1, 0.1, 0.8), nr = 2, nc = 2, byrow = TRUE)
print(h)
...
conformity_simple_contagion <- function(t, comp, parms){
  with(as.list(c(comp)),{
    dcomp1 <- comp[1] * (
                (h[1, 1] * (1 - (comp[1] - comp[3]))) +
                (h[1, 2] * (1 - (comp[2] - comp[4]))) #+ birth_a + death_a
              )
    dcomp2 <- comp[2] * (
                (h[2, 2] * (comp[2] - comp[4])) +
                (h[2, 1] * (comp[1] - comp[3])) #+ birth_a + death_a
              )
    dcomp3 <- comp[3] * (
                (h[1, 1] * (comp[3] - comp[1])) +
                (h[1, 2] * (comp[4] - comp[2])) #+ birth_A + death_A
              )
    dcomp4 <- comp[4] * (
                (h[2, 2] * (comp[4] - comp[2])) +
                (h[2, 1] * (comp[3] - comp[1])) #+ birth_A + death_A
              )

    res <- c(dcomp1,dcomp2,dcomp3,dcomp4)
...
comp_IC <- c(0.5, 0.35, 0.5, 0.65)
# Must pass params to lsoda? Hack bc that seems to be the case.
parms <- c()
model.out <- as.data.frame(lsoda(comp_IC, times, conformity_simple_contagion, parms))
names(model.out) <- c("time", "n_1(a)", "n_2(a)", "n_1(A)", "n_2(A)")

plot(model.out[,"time"], 
     model.out[,"n_1(a)"]/(comp_IC[1] + comp_IC[3]), 
     type="l", lwd=2, col="blue", 
     xaxs="i", xlim=c(0,50), ylim=c(0,1),
     xlab="Time", ylab="Fraction Adopting")

lines(model.out[,"time"], 
      model.out[,"n_2(a)"]/(comp_IC[2] + comp_IC[4]), 
